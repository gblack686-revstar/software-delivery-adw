...
# Find the project root (parent of adws directory)
_module_dir = os.path.dirname(os.path.abspath(__file__))  # adws/adw_modules
_adws_dir = os.path.dirname(_module_dir)  # adws
_project_root = os.path.dirname(_adws_dir)  # project root
...
def prompt_claude_code(request: AgentPromptRequest) -> AgentPromptResponse:
    ...
    # Build command - use stdin for prompts to bypass Windows 8K command-line limit
    ...
    try:
        ...
        with open(request.output_file, "w", encoding='utf-8') as output_f:
            ...
            # Pass prompt via stdin to bypass Windows command-line length limits
            result = subprocess.run(
                cmd,
                ...
                text=True,
                encoding='utf-8',  # Ensure UTF-8 for Windows compatibility
                env=env,
