...
class AWSResourceType(str, Enum):
    """AWS resource types for infrastructure tracking."""
    ...
    PARAMETER_STORE_PARAM = "parameter_store_param"
    IAM_ROLE = "iam_role"
    VPC = "vpc"
...
class AWSResource(BaseModel):
    ...
    resource_name: str
    resource_id: Optional[str] = None  # ARN or physical ID after deployment
    properties: Dict[str, Any] = Field(default_factory=dict)
    created_at: Optional[datetime] = None
    region: str = "us-east-1"


class CDKStackInfo(BaseModel):
    """CDK stack information."""

    stack_name: str
    stack_id: Optional[str] = None  # Stack ID after deployment
    status: Literal["pending", "deploying", "deployed", "failed", "destroyed"] = "pending"
    resources: List[AWSResource] = Field(default_factory=list)
    outputs: Dict[str, str] = Field(default_factory=dict)  # Stack outputs
    parameters: Dict[str, str] = Field(default_factory=dict)  # Stack parameters
    region: str = "us-east-1"
...
class InfrastructureConfig(BaseModel):
    """Infrastructure configuration for AWS deployments."""
    ...
    parameter_store_prefix: str = "/sdaw"  # Prefix for all parameters
    secrets_manager_prefix: str = "sdaw"  # Prefix for all secrets

    # CDK configuration
    cdk_config_path: Optional[str] = None  # Path to generated CDK config YAML
    cdk_output_dir: str = "cdk_output"  # Directory for CDK synthesized templates

    # Resource naming
    resource_prefix: str  # Prefix for all AWS resources (e.g., "clientA-dev")

    # Stack info
    stacks: List[CDKStackInfo] = Field(default_factory=list)


class InfrastructureTestResult(BaseModel):
    """Result from infrastructure testing."""

    test_name: str
    passed: bool
    resource_type: Optional[AWSResourceType] = None
...
# ============================================================================
# Enhanced ADW State Data with AWS Support
# ============================================================================
...
class ADWStateData(BaseModel):
    ...
    Enhanced with AWS infrastructure tracking.
    """
    ...
    infrastructure_deployed: bool = False
    infrastructure_tested: bool = False
    infrastructure_test_results: List[InfrastructureTestResult] = Field(default_factory=list)
